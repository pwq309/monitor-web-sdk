...
var Coronacore = new _core2.default();
...
var extractXhrAttr = function extractXhrAttr(xhrObj) {
    if (!_utils2.default.isXMLHttpRequest(xhrObj)) {
        return xhrObj;
    }
    var result = {};
...
var extractEventAttr = function extractEventAttr(eventObj) {
    if (!_utils2.default.isEvent(eventObj)) {
        return eventObj;
    }
    var result = {};
...
var extractAttr = function extractAttr(obj) {
    if (_utils2.default.isXMLHttpRequest(obj)) {
        return extractXhrAttr(obj);
    }
    if (_utils2.default.isEvent(obj)) {
...
var setUser = function setUser(user) {
    if (!_utils2.default.isString(user)) {
...
var setGlobalTags = function setGlobalTags(tag) {
    if (!_utils2.default.isPlainObject(tag)) {
        // eslint-disable-next-line no-console
...
var setGlobalOnError = function setGlobalOnError(cb) {
    if (!_utils2.default.isFunction(cb) && !_utils2.default.isNull(cb)) {
        // eslint-disable-next-line no-console
...
var info = function info(msg, tags) {
    ...
    if (typeof msg === 'string') {
        ...
        msg = extractAttr(msg);
        if (_utils2.default.isPlainObject(msg)) {
            tags = _utils2.default.isPlainObject(tags) ? objectMerge(tags, msg) : msg;
        }
    }
    payloads.tags = tags || '';
...
var error = function error(msg, tags, active, type) {
    ...
    if (_utils2.default.isBoolean(active)) {
    ...
    if (JSON.stringify(errPayloads) !== '{}') {
        if (_utils2.default.isPlainObject(tags)) {
...
var onunhandledrejectionCallback = function onunhandledrejectionCallback(reason) {
    ...
    if (_utils2.default.isPlainObject(reason)) {
        var errorMessage = _polymorphic2.default.formatPlainObjectMessage(reason);
        error(errorMessage, reason, false, 'unhandledrejection');
        // 如果 reject 出来的是个数组，为了友好上报放到 tag 信息里
    } else if (_utils2.default.isArray(reason)) {
        error('未处理的 unhandledrejection 事件', {
            type: 'array',
            data: JSON.stringify(reason)
        }, false, 'unhandledrejection');
        // 如果 reject 出来的是个函数，为了友好上报放到 tag 信息里
    } else if (_utils2.default.isFunction(reason)) {
        error('未处理的 unhandledrejection 事件', {
            type: 'function',
            data: reason.toString()
        }, false, 'unhandledrejection');
        // 如果 reject 出来的是个boolean，为了友好上报放到 tag 信息里
    } else if (_utils2.default.isBoolean(reason)) {
        error('未处理的 unhandledrejection 事件', {
            type: 'boolean',
            data: reason
        }, false, 'unhandledrejection');
        // 如果 reject 出来的是个 XMLHttpRequest 对象，提取接口信息聚合
    } else if (_utils2.default.isXMLHttpRequest(reason)) {
...
var corona = function corona(params) {
    ...
    Coronacore.config(params.id, env, debug, _config2.default).install();
